import 'dart:io';
import 'package:ez_deliver_tracksure/gps/mapgps.dart'; // ตรวจสอบ Path และชื่อไฟล์ให้ถูกต้องC:\ez_deliver_tracksure\lib\gps\mapgps.dart
import 'login.dart';
    ...
    try {
      ...
      UserCredential userCredential = await FirebaseAuth.instance
          .createUserWithEmailAndPassword(
              email: emailForAuth, password: _passwordController.text.trim());
      ...
      if (user != null) {
        ...
        if (_profileImage != null) {
          profileImageUrl = await _uploadImage(_profileImage!, user.uid);
        }

        if (_userType == 'ผู้ใช้') {
          Map<String, dynamic> customerData = {
            'customer_name': _usernameController.text.trim(), //คือชื่อ-สกุล
            'customer_phone': _phoneController.text.trim(), //คือเบอร์โทรศัพท์
            'customer_address': _addressController.text.trim(), //คือที่อยู่
            'profile_image_url': profileImageUrl,
            'gps_location': _gpsController.text.trim(), //คือพิกัด GPS
            'createdAt': FieldValue.serverTimestamp(), //คือ
          };
          ...
          if (_vehicleImage != null) {
            vehicleImageUrl = await _uploadImage(_vehicleImage!, user.uid);
          }
          Map<String, dynamic> riderData = {
            ...
            'status': 'pending_approval', //คือสถานะรอการอนุมัติ
          };
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('เกิดข้อผิดพลาดที่ไม่รู้จัก: $e')));
    } finally {
      if (mounted) {
            ...
            Padding(
                ...
                child: Text('TRACK-SURE',
                    style: GoogleFonts.alumniSansInlineOne(
                        fontSize: 65, color: Colors.white, letterSpacing: 3))),
            Expanded(
                child: Container(
                    ...
                    child: SingleChildScrollView(
                        child: Form(
                            ...
                            child: Column(children: [
                              ...
                              const SizedBox(height: 20),
                              _buildTextField(_usernameController, 'ชื่อ-สกุล',
                                  Icons.person),
                              ...
                              // =======================================================
                              // ==> โครงสร้างใหม่ที่แก้ไขแล้ว <==
                              // =======================================================
                              if (_userType == 'ผู้ใช้') ...[
                                // --- Fields สำหรับ "ผู้ใช้" เท่านั้น ---
                                Padding(
                                  padding:
                                  ...
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                        labelText:
                                            'พิกัด GPS (แตะเพื่อเลือกบนแผนที่)',
                                ...
                                Padding(
                                  ...
                                  child: TextFormField(
                                    ...
                                    decoration: InputDecoration(
                                        labelText:
                                            'ที่อยู่ (พิมพ์เพื่อค้นหา หรือเลือกจากแผนที่)',
                                        prefixIcon:
                              ...
                              if (_userType == 'ไรเดอร์') ...[
                                // --- Fields สำหรับ "ไรเดอร์" เท่านั้น ---
                                const SizedBox(height: 15),
                                Divider(color: Colors.grey[300]),
                                Padding(
                                    padding: const EdgeInsets.symmetric(
                                        vertical: 8.0),
                                    child: Text("ข้อมูลเพิ่มเติมสำหรับไรเดอร์",
                                        style: GoogleFonts.prompt(
                                            fontSize: 16,
                                            fontWeight: FontWeight.bold,
                                            color: Colors.black54))),
                                _buildTextField(_licensePlateController,
                                    'หมายเลขทะเบียนรถ', Icons.motorcycle),
                                const SizedBox(height: 20),
    ...
    return Column(children: [
      GestureDetector(
          ...
          child: CircleAvatar(
              ...
              child: image == null
                  ? Icon(isProfile ? Icons.person_add_alt_1 : Icons.camera_alt,
                      color: Colors.grey[800], size: isProfile ? 40 : 50)
                  : null)),
